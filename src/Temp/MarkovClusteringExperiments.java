 ...
 * @author G.A.P. II
 */
public class MarkovClusteringExperiments {
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 0; datasetId < datasetProfiles.length; datasetId++) {
            ...
            int bestChecks = -1;
            double bestClusterThreshold = Double.MIN_VALUE;
            double bestMatrixSimThreshold = Double.MIN_VALUE;
            ...
            for (RepresentationModel repModel : RepresentationModel.values()) {
                ...
                for (SimilarityMetric simMetric : simMetrics) {
                    ...
                    IEntityMatching em = new ProfileMatcher(repModel, simMetric);
                    SimilarityPairs simPairs = em.executeComparisons(copyOfBlocks, profiles);

                    for (double th = 0.1; th < 1.0; th += 0.1) {
                        for (int checks = 1; checks < 4; checks++) {
                            for (double clusterThreshold = 0.1; clusterThreshold < 1.00; clusterThreshold += 0.05) {
                                for (double matrixSimThreshold = 0.1; matrixSimThreshold < 1.00; matrixSimThreshold += 0.05) {
                                    MarkovClustering ec = new MarkovClustering();
                                    ec.setClusterThreshold(clusterThreshold);
                                    ec.setMatrixSimThreshold(matrixSimThreshold);
                                    ec.setSimilarityChecksLimit(checks);
                                    ...
                                    if (bestFMeasure < currentFMeasure) {
                                        bestChecks = checks;
                                        bestClusterThreshold = clusterThreshold;
                                        bestFMeasure = currentFMeasure;
                                        bestMatrixSimThreshold = matrixSimThreshold;
            ...
            System.out.println("\n\n\n\n\nBest configuration...");
            System.out.println("Best Checks\t:\t" + bestChecks);
            System.out.println("Best Cluster Threshold\t:\t" + bestClusterThreshold);
            System.out.println("Best F-Measure\t:\t" + bestFMeasure);
            System.out.println("Best Matrix Sim Threshold\t:\t" + bestMatrixSimThreshold);
